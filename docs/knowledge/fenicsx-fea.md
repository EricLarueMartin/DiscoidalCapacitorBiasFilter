...
- The default `mirror_half` strategy meshes one physical end through the center of the middle plate. The center cut uses the weak form's natural zero-normal-flux boundary condition, energy is doubled, and sampled fields are mirrored back to the full-stack browser grid. `full_stack` remains available for regression comparisons.
...
The optional `Repeat interior FEA cell` checkbox selects `end_repeat_approx`. It solves two axial regions: the physical end through the center of the first bias plate, and the first bias-plate center through the center of the second ground plate. The far end is the exact global mirror of the first region. The interior half-cell is copied and mirror-copied across alternating bias and ground center planes until the far-end region begins. The reconstructed ordinary field/capacitance result is marked approximate.
