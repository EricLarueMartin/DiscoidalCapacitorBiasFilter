...
The web interface also uses provisional dielectric-strength values to set the lower and upper range for clearance-like geometry sliders. It rounds the minimum clearance upward from `bias_voltage / dielectric_strength`, then sets the slider maximum to 10 times that rounded minimum. Custom and generic high-K screening presets use a fixed `0.4 mm` to `4 mm` range until a real material is selected. After a field solve, the web interface reports the neighborhood-supported electric field in the washer and epoxy/fill regions and displays `E_breakdown / E_max` as a first-pass design margin where a placeholder breakdown strength is available. The raw point maximum remains available as a mesh-artifact diagnostic. The epoxy/fill bucket includes the core-to-ground dielectric gap around the conductive core and the rounded-edge pockets outside the flat washer slabs.
