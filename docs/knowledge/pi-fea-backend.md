...
- The backend records a symmetry plan in each result. The FEniCSx worker defaults to the `end_repeat_approx` screening strategy; exact `mirror_half` remains the unchecked reference option, and the finite-difference screen still solves its full structured grid.
- `full_stack` remains the FEniCSx reference strategy for same-geometry implementation and regression checks.
- The Solver tab enables `Repeat interior FEA cell` by default. It solves end-to-first-bias-center and bias-center-to-second-ground-center regions, then uses copy/mirror reconstruction across successive plate-center planes. One-pair stacks fall back to exact `mirror_half`, and the reconstructed result remains explicitly labeled approximate. Clearing the checkbox selects exact `mirror_half`.
- With that checkbox enabled, the three-bias Cpar worker keeps its exact middle-bias mirror but truncates at the neighboring bias center. The neighbor electrode retains its solve voltage and the exposed bias-OD-to-tube annulus on the cut is grounded, removing the end washer/ground region. Unchecked mode retains the original exact mirror-half Cpar domain.
