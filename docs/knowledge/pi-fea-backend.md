...
- The backend records a symmetry plan in each result. The FEniCSx worker defaults to exact `mirror_half` symmetry, doubles half-domain energy, and reconstructs the full browser field grid; the finite-difference screen still solves its full structured grid.
- `full_stack` remains the FEniCSx reference strategy for same-geometry implementation and regression checks.
- The Solver tab has an opt-in `Repeat interior FEA cell` checkbox. It solves end-to-first-bias-center and bias-center-to-second-ground-center regions, then uses copy/mirror reconstruction across successive plate-center planes. One-pair stacks fall back to exact `mirror_half`, and the reconstructed result remains explicitly labeled approximate.
